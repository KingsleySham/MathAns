...
- **Morning weather goes to the whole board (VHP decision, Aug 2026):** it is sent to every opted-in prefect on a duty day, not just that day's team — a warning that moves assembly indoors affects anyone who might turn up. Delivered via the already-approved `prefect_notice` template rather than `prefect_duty_reminder_weather`, whose body ("You'll be having duty at {{gate}} today") would tell the thirty-odd non-rostered prefects they were on duty; the rostered few already had their gate and time in the evening reminder. The cron stays at 22:00 UTC — Hobby fires anywhere within the scheduled hour, so 23:00 UTC would mean a 07:00–08:00 HK window that can land after the 7:45 duty has started, and arriving before prefects leave home matters more than hitting 07:00 exactly.
