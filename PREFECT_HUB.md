...
- **Morning weather goes to the whole board (VHP decision, Aug 2026):** it is sent to every opted-in prefect on a duty day, not just that day's team — a warning that moves assembly indoors affects anyone who might turn up — and keeps the `prefect_duty_reminder_weather` template (VHP's call). Consequence to watch: `{{gate}}`/`{{time}}`/`{{day}}` now describe *the day's duty* rather than the reader's own assignment, so **the Meta template body must be reworded out of the second person**. As approved it opens "You'll be having duty at *{{gate}}* today", which tells ~35 non-rostered prefects to turn up. The wording is edited and re-reviewed in Meta, so the repo cannot enforce it — see `api/whatsapp/README.md`.
