...
- **Handbook read-check (VHP request, Aug 2026):** at the start of each change, after the intro template goes out, the VHP starts a *round* from `/prefects/status/update` → WhatsApp tab → Handbook check. Every opted-in prefect gets a `prefect_notice` asking them to open the handbook and send back the 6-digit code shown on `/prefects/handbook` — an embed to place inside the Notion handbook page. Sending the code records them as having read it; the admin panel lists who has and has not.
  - The code is TOTP in all but name: HMAC-SHA256 over a 10-minute counter, truncated to six digits, written out in `lib/prefect-handbook.js` rather than pulled from a library. The previous window still verifies, so a prefect who is slow switching apps is not rejected; **future windows never verify**, since nobody can have read a code that has not been shown.
  - **What it proves, honestly:** the handbook page is a public Notion embed with no login, so every prefect sees the same code and the first one to open it can paste it into the group chat. This is an attendance code, not authentication. The rotation defends against a code being passed around *days later*, which is the realistic failure — not against a determined forger. Starting a round rotates the secret, so every code shown before that moment stops working.
  - A bare six-digit message is the discriminator in `handlePrefectText`: an absence reason is prose, so the two flows cannot collide, and the code is only claimed when a round is actually open. A prefect mid-absence-reason can still confirm the handbook without losing the question.
  - The secret lives in Redis (`prefect:handbook-secret`), made on first use — no extra env var. State is `prefect:handbook` = `{ round, confirmed }`, cleared when a round starts.
  - The public code endpoint is folded into `api/prefects/status.js` as `?op=handbook-code` (rewritten to `/api/prefects/handbook-code`), and the admin ops into `api/whatsapp/tasks.js` — the deployment is at the Hobby cap of 12 serverless functions. The Redis state is in `lib/prefect-handbook-store.js` so the public status endpoint can serve the code without importing the whole WhatsApp stack.
- **A zero ETA means out of service (VHP, Aug 2026):** a first-train `ttnt` of `0` used to fall under the per-line threshold and produce no alert, so a station with nothing scheduled looked exactly like one with a train two minutes out — silence reading as "fine" when it meant the opposite. Inside the 06:30–08:30 window the line is running, so a reported zero is the feed saying that platform has nothing timetabled; `evaluateLine` now returns an amber *"Out of service"* alert when **every** first-train ETA is zero. One direction at zero while the other has a train is not a closure, and the zero is ignored for the threshold comparison. Only the three school-serving lines have their ETAs read at all — the other seven are sampled for network-wide `status`/`isdelay` only, which was implicit before (`worst >= undefined` is always false) and is now an explicit early return, since otherwise the new check would fire on the Disneyland Resort Line.
