...
// The sign-off only uses a template when one is named; most of this file
// exercises that path, and the default is covered on its own below.
process.env.WHATSAPP_CLASH_DONE_TEMPLATE = 'class_clash_done';
...
const { addDays, mondayOf, shortDate } = await import('./clash-schedule.js');
const { NA } = await import('./clash-flow.js');
...
// Dates are worked out from the real clock, because the flow reads it: next
// week's Tuesday to Thursday is always far enough ahead that the make-up rule
// (same week, before the clash) has room, whatever day the suite runs on.
const hkToday = () => new Date(Date.now() + 8 * 3600 * 1000).toISOString().slice(0, 10);
const nextWeek = (weekday) => addDays(mondayOf(hkToday()), 7 + weekday - 1);
const TUE = nextWeek(2);
const THU = nextWeek(4);
...
test('"clash" asks for the event, not for lesson codes', async () => {
  seed();
  const claimed = await handleClashWebhook(text(KINGSLEY, 'clash'));

  assert.equal(claimed, true);
  assert.match(lastText(), /What's the event/i);
  assert.match(lastText(), /16\/8-19\/8/, 'the date format has to be shown or it will not be typed');
});

test('the event alone produces the clashes, the make-ups, and the template', async () => {
  seed();
  await handleClashWebhook(text(KINGSLEY, 'clash'));

  // One line, one event: everything else is worked out from the timetable.
  await handleClashWebhook(text(KINGSLEY, `HKYAS, ${shortDate(TUE)}-${shortDate(THU)}`));

  const summary = lastText();
  assert.match(summary, /Japanese/, 'Tuesday is inside the event');
  assert.match(summary, /Piano/);
  assert.match(summary, /Math/);
  assert.match(summary, /→/, 'each clash carries where it is going instead');
  assert.match(summary, /Reply \*yes\*/, 'nothing is sent until a human agrees');
  assert.equal(templates().length, 0);

  await handleClashWebhook(text(KINGSLEY, 'yes'));
  ...
  assert.match(body.tutorial_1, /^1️⃣ Tuesday 6pm - Japanese/);
  assert.match(body.tutorial_3, /^3️⃣ Thursday 6:30pm - Math/);
  assert.equal(body.tutorial_4, NA);
  assert.match(body.events, /^HKYAS \(/);
  assert.equal(body.tutorial_arranged, NA);
...
test('a make-up lands in the same week as the lesson and before it', async () => {
  seed();
  await handleClashWebhook(text(KINGSLEY, 'clash'));
  await handleClashWebhook(text(KINGSLEY, `Speech Day, ${shortDate(THU)}`));
  await handleClashWebhook(text(KINGSLEY, 'yes'));

  const [{ makeup, date }] = JSON.parse(store.get(`clash:case:${JSON.parse(store.get('clash:open'))[0]}`)).lessons;
  assert.equal(date, THU, 'Thursday Math is what a Thursday event costs');
  assert.ok(makeup.date, 'and there is room that week');
  assert.equal(mondayOf(makeup.date), mondayOf(THU), 'same week');
  assert.ok(makeup.date < THU, 'before the day it was missed');
});

test('skipping a home class is asked, not assumed', async () => {
  // Math is at a centre that only opens Tuesday 6pm — when Japanese is on at
  // home. The one slot that exists costs a lesson, so it is a question.
  seed();
  store.set('clash:lessons', JSON.stringify([
    { code: '1', name: 'Japanese', weekday: 2, start: '6pm', end: '7pm', homeClass: true },
    { code: '3', name: 'Math', weekday: 4, start: '6pm', end: '7:30pm', makeupSlots: 'Tue 6pm-7:30pm' },
  ]));

  await handleClashWebhook(text(KINGSLEY, 'clash'));
  await handleClashWebhook(text(KINGSLEY, `Speech Day, ${shortDate(THU)}`));

  assert.match(lastText(), /only fits if \*Japanese\* at home is skipped/i);
  assert.match(lastText(), /Reply \*yes\*/);
  assert.match(lastText(), /\*no\* to keep Japanese/);
  assert.equal(templates().length, 0, 'nothing is sent on the assumption');

  await handleClashWebhook(text(KINGSLEY, 'yes'));
  assert.match(paramsOf(templates()[0], 'body').tutorial_1, /skips Japanese/,
    'and everyone is told what it cost');
});

test('"no" keeps the home class and leaves the make-up unscheduled', async () => {
  seed();
  store.set('clash:lessons', JSON.stringify([
    { code: '1', name: 'Japanese', weekday: 2, start: '6pm', end: '7pm', homeClass: true },
    { code: '3', name: 'Math', weekday: 4, start: '6pm', end: '7:30pm', makeupSlots: 'Tue 6pm-7:30pm' },
  ...
  await handleClashWebhook(text(KINGSLEY, 'clash'));
  await handleClashWebhook(text(KINGSLEY, `Speech Day, ${shortDate(THU)}`));
  await handleClashWebhook(text(KINGSLEY, 'no'));

  const body = paramsOf(templates()[0], 'body');
  assert.match(body.tutorial_1, /no free slot/, 'the make-up is dropped, not forced');
  assert.doesNotMatch(body.tutorial_1, /skips/);
});

test('"no" is not an answer when nothing was going to be skipped', async () => {
  seed();
  await handleClashWebhook(text(KINGSLEY, 'clash'));
  await handleClashWebhook(text(KINGSLEY, `Speech Day, ${shortDate(THU)}`));
  await handleClashWebhook(text(KINGSLEY, 'no'));

  assert.equal(templates().length, 0, 'a stray "no" must not send the clash');
  assert.match(lastText(), /Reply \*yes\*/);
});

test('an event that clashes with nothing sends nothing', async () => {
  seed();
  await handleClashWebhook(text(KINGSLEY, 'clash'));
  // A Sunday: no tutorial runs then.
  await handleClashWebhook(text(KINGSLEY, `Church fair, ${shortDate(addDays(THU, 3))}`));

  assert.match(lastText(), /Nothing in your timetable clashes/i);
  assert.equal(templates().length, 0);
});

test('an event with no date is asked for one', async () => {
  seed();
  await handleClashWebhook(text(KINGSLEY, 'clash'));
  await handleClashWebhook(text(KINGSLEY, 'HKYAS'));

  assert.match(lastText(), /I need the date too/i);
  assert.equal(templates().length, 0, 'nothing goes out on a guess');
});

test('"stop" abandons a half-finished clash', async () => {
  seed();
  await handleClashWebhook(text(KINGSLEY, 'clash'));
  await handleClashWebhook(text(KINGSLEY, `HKYAS, ${shortDate(THU)}`));
...
async function openOne(codes = ['1', '3']) {
  seed();
  const out = await openClash({ codes, events: [{ name: 'HKYAS', startDate: TUE, endDate: THU }], via: 'web' });
  sends.length = 0;
  return out.case;
}
...
test('two lessons can be ticked off in one reply', async () => {
  // WhatsApp lists are single-select, so typing the codes is how several get
  // sorted at once — the same "1,3" language the bot already asks for.
  const c = await openOne(['1', '2', '3']);
  await handleClashWebhook(button(MUM, `CLASH_DONE:${c.id}`));
  sends.length = 0;

  await handleClashWebhook(text(MUM, '1,2'));

  const resent = templates();
  assert.equal(resent.length, 3, 'one message to all three, not one per lesson');
  const body = paramsOf(resent[0], 'body');
  assert.match(body.tutorial_1, /^3️⃣ Thursday 6:30pm - Math/, 'only the untouched lesson is still a clash');
  assert.match(body.tutorial_arranged, /1️⃣ Tuesday 6pm - Japanese/);
  assert.match(body.tutorial_arranged, /2️⃣ Wednesday 6pm - Piano/);
});

test('typing every remaining code finishes the clash in one go', async () => {
  const c = await openOne(['1', '3']);
  await handleClashWebhook(button(DAD, `CLASH_DONE:${c.id}`));
  sends.length = 0;

  await handleClashWebhook(text(DAD, '1 3'));

  assert.deepEqual([...new Set(templates().map((s) => s.template.name))], ['class_clash_done']);
  assert.deepEqual(JSON.parse(store.get('clash:open')), []);
});

test('a code that is not part of the clash is named, and the rest still count', async () => {
  const c = await openOne(['1', '3']);
  await handleClashWebhook(button(MUM, `CLASH_DONE:${c.id}`));
  sends.length = 0;
  ...
  assert.equal(templates().length, 3, 'lesson 1 is still ticked off');
  assert.match(lastText(), /don't have \*2\* on this clash/);
});

test('codes only mean "arranged" while the picker is open', async () => {
  // Otherwise any stray number from a parent would tick a lesson off.
  const c = await openOne();
  await handleClashWebhook(text(MUM, '1'));

  assert.equal(templates().length, 0, 'nothing is marked');
  assert.match(lastText(), /Send \*clash\*/);
  const stored = JSON.parse(store.get(`clash:case:${c.id}`));
  assert.equal(stored.lessons.every((l) => !l.fixed), true);
});

test('the picker says how to tick off several at once', async () => {
  const c = await openOne();
  await handleClashWebhook(button(MUM, `CLASH_DONE:${c.id}`));

  assert.match(interactives()[0].interactive.body.text, /reply with the codes/i);
});
...
test('picking a lesson re-sends the alert to all three with it moved to Arranged', async () => {
  ...
  const body = paramsOf(resent[0], 'body');
  assert.match(body.tutorial_1, /^3️⃣ Thursday 6:30pm - Math/, 'only the outstanding lesson is still a clash');
  assert.equal(body.tutorial_2, NA);
  assert.match(body.tutorial_arranged, /^1️⃣ Tuesday 6pm - Japanese/);
...
test('the last lesson sends the sign-off template instead, with no buttons', async () => {
  ...
  assert.match(body.tutorial_1, /^1️⃣ Tuesday 6pm - Japanese/, 'the sign-off lists what was arranged');
  assert.match(body.tutorial_2, /^2️⃣ Wednesday 6pm - Piano/);
  assert.equal(body.tutorial_3, NA);
  assert.deepEqual(JSON.parse(store.get('clash:open')), [], 'and it drops off the open list');
});

test('with no sign-off template named, the news goes out as plain text', async () => {
  // Kingsley's call: the sign-off is plain text unless a template is approved
  // and named, so nothing should be attempting one by default.
  const named = process.env.WHATSAPP_CLASH_DONE_TEMPLATE;
  delete process.env.WHATSAPP_CLASH_DONE_TEMPLATE;
  try {
    const c = await openOne(['1']);
    await handleClashWebhook(listReply(MUM, `CLASH_FIX:${c.id}:1`));

    assert.equal(templates().length, 0, 'no template is even attempted');
    assert.equal(texts().length, 3, 'all three are told, in plain text');
    assert.match(lastText(), /All clashed tutorial\(s\) have been arranged/);
  } finally {
    process.env.WHATSAPP_CLASH_DONE_TEMPLATE = named;
  }
});
...
test('six digits from a parent never reaches the handbook check', async () => {
  ...
  // And mid-clash it is read as what it is: not a date.
  sends.length = 0;
  await handleClashWebhook(text(MUM, 'clash'));
  await handleClashWebhook(text(MUM, '123456'));
  assert.match(lastText(), /I need the date too/);
});
...
test('the page and WhatsApp open identical cases', async () => {
  seed();
  const out = await openClash({ codes: ['2'], events: [{ name: 'Speech Day', startDate: THU }], via: 'web' });

  assert.equal(out.ok, true);
  assert.equal(out.sent, 3);
  assert.equal(out.case.createdVia, 'web');
  assert.match(paramsOf(templates()[0], 'body').tutorial_1, /^2️⃣ Wednesday 6pm - Piano/);
});

test('a lesson picked by hand that the event misses still gets a date and a make-up', async () => {
  // Piano is a Wednesday; the event is the Thursday. Ticking it anyway has to
  // mean something, so it is hung on its next occurrence.
  seed();
  const out = await openClash({ codes: ['2'], events: [{ name: 'Speech Day', startDate: THU }], via: 'web' });

  const [piano] = out.case.lessons;
  assert.equal(piano.code, '2');
  assert.ok(piano.date, 'a date, so "the same week, before it" means something');
  assert.equal(mondayOf(piano.makeup.date || piano.date), mondayOf(piano.date));
});

test('a skipped home class is still listed, and still needs ticking off', async () => {
  // "Skip class, notify tutor" is an instruction to a person, so the lesson
  // stays in the clash and in the picker until someone says it is done.
  seed();
  const out = await openClash({
    codes: ['1', '3'],
    skips: ['1'],
    events: [{ name: 'Speech Day', startDate: THU }],
    via: 'web',
  });

  const body = paramsOf(templates()[0], 'body');
  const lines = [body.tutorial_1, body.tutorial_2];
  assert.ok(lines.some((l) => /Japanese → Skip class, notify tutor/.test(l)), 'it is still in the message');
  assert.ok(lines.some((l) => /Math/.test(l)));

  sends.length = 0;
  await handleClashWebhook(button(MUM, `CLASH_DONE:${out.case.id}`));
  const rows = interactives()[0].interactive.action.sections[0].rows;
  assert.deepEqual(rows.map((r) => r.title), ['1️⃣ Japanese', '3️⃣ Math'],
    'both are still outstanding — the tutor has not been told yet');
});
