...
// Japanese occupies — and online on the Wednesday, which nothing does.
const ONLINE_MATH = {
  ...LESSONS[2],
  makeupSlots: parseSlots('Tue 5pm-6:30pm'),
  onlineSlots: parseSlots('Wed 8pm-9:30pm'),
};
const ONLINE_TIMETABLE = [{ ...LESSONS[0], start: at(17), end: at(19) }, ONLINE_MATH];
const ONLINE_EVENTS = [{ name: 'Speech Day', startDate: THU }];

test('online hours are ignored until online is chosen', () => {
  ...
  assert.equal(makeup.date, null, 'the Wednesday online hour is not on offer by default');
});

test('choosing online opens up the hours the centre only runs online', () => {
  const [makeup] = proposeMakeups(detectClashes(ONLINE_TIMETABLE, ONLINE_EVENTS), {
...
test('online is chosen per lesson, not for everything at once', () => {
  const other = { ...LESSONS[1], makeupSlots: parseSlots('Tue 6pm'), onlineSlots: parseSlots('Mon 8pm') };
  const timetable = [{ ...LESSONS[0], start: at(17), end: at(19) }, ONLINE_MATH, other];
  const events = [{ name: 'Speech Day', startDate: THU, endDate: addDays(THU, -1) }];
  const clashes = detectClashes(timetable, [{ name: 'Speech Day', startDate: WED, endDate: THU }]);
  const makeups = proposeMakeups(clashes, {
    lessons: timetable, events, today: SAT_BEFORE, nowMinutes: at(9), online: new Set(['3']),
  });

  const piano = makeups.find((m) => m.code === '2');
  assert.notEqual(piano?.date, MON, 'Piano was not opted in, so its Monday online hour stays shut');
});

test('in person beats online when both are equally near', () => {
  const both = {
    ...LESSONS[2],
    makeupSlots: parseSlots('Wed 6:30pm'),
    onlineSlots: parseSlots('Wed 6:30pm'),
  };
