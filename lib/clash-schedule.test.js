...
// Maths at a centre that runs it in person only on the Tuesday — which
// Japanese occupies — and will also take it online, which has no timetable.
const ONLINE_MATH = {
  ...LESSONS[2],
  makeupSlots: parseSlots('Tue 5pm-6:30pm'),
  canBeOnline: true,
};
const ONLINE_TIMETABLE = [{ ...LESSONS[0], start: at(17), end: at(19) }, ONLINE_MATH];
const ONLINE_EVENTS = [{ name: 'Speech Day', startDate: THU }];

test('the centre\'s fixed hours still bind until online is chosen', () => {
  const [makeup] = proposeMakeups(detectClashes(ONLINE_TIMETABLE, ONLINE_EVENTS), {
    lessons: ONLINE_TIMETABLE, events: [], today: SAT_BEFORE, nowMinutes: at(9),
  });
  assert.equal(makeup.date, null, 'its only in-person hour is taken, and online was not asked for');
});

test('choosing online frees the make-up from the centre\'s timetable', () => {
  // Online has no hours of its own, so the lesson simply goes at its usual
  // time on the nearest day that is free — like a tutor who moves to suit.
  const [makeup] = proposeMakeups(detectClashes(ONLINE_TIMETABLE, ONLINE_EVENTS), {
    lessons: ONLINE_TIMETABLE, events: [], today: SAT_BEFORE, nowMinutes: at(9), online: new Set(['3']),
  });
  assert.equal(makeup.date, WED, 'the day before the clash');
  assert.equal(makeup.start, at(18, 30), 'at its own hour, since online has none of its own');
  assert.equal(makeup.online, true, 'and the proposal says how it is attended');
});

test('online is chosen per lesson, not for everything at once', () => {
  const other = { ...LESSONS[1], makeupSlots: parseSlots('Tue 6pm'), canBeOnline: true };
  const timetable = [{ ...LESSONS[0], start: at(17), end: at(19) }, ONLINE_MATH, other];
  const clashes = detectClashes(timetable, [{ name: 'Speech Day', startDate: WED, endDate: THU }]);
  const makeups = proposeMakeups(clashes, {
    lessons: timetable, events: [{ name: 'Speech Day', startDate: WED, endDate: THU }],
    today: SAT_BEFORE, nowMinutes: at(9), online: new Set(['3']),
  });

  const piano = makeups.find((m) => m.code === '2');
  assert.notEqual(piano?.online, true, 'Piano was not opted in, so it stays bound to Tuesday 6pm');
});

test('in person beats online when both are equally near', () => {
  // The centre's own Wednesday hour is free, so there is no reason to be
  // online even though online was allowed.
  const both = { ...LESSONS[2], makeupSlots: parseSlots('Wed 6:30pm'), canBeOnline: true };
  const [makeup] = proposeMakeups(detectClashes([both], ONLINE_EVENTS), {
    lessons: [both], events: [], today: SAT_BEFORE, nowMinutes: at(9), online: new Set(['3']),
  });
  assert.equal(makeup.date, WED);
  assert.equal(makeup.online, undefined, 'no reason to be online when the room is free');
});

test('a lesson with no fixed hours at all is unaffected by the online choice', () => {
  const [makeup] = proposeMakeups(detectClashes([LESSONS[2]], ONLINE_EVENTS), {
    lessons: [LESSONS[2]], events: [], today: SAT_BEFORE, nowMinutes: at(9), online: new Set(['3']),
  });
  assert.equal(makeup.date, WED, 'still its own hour, the day before');
});
...
test('the scarce slot goes to the lesson that has no other option', () => {
  // Reported from the page: Chemistry (a tutor who moves to suit, so any
  // earlier day at its own hour would do) took the one Tuesday hour and
  // displaced Japanese, leaving Maths — a centre whose only slot that week IS
  // that Tuesday hour — with nothing. It should be the other way round.
  const timetable = [
    { code: '1', name: 'Japanese', weekday: 2, start: at(17), end: at(19), homeClass: true },
    { code: '3', name: 'Chemistry', weekday: 3, start: at(17, 45), end: at(19, 15) },
    {
      code: '4', name: 'Maths', weekday: 4, start: at(18, 30), end: at(20),
      makeupSlots: parseSlots('Tue 5pm-6:30pm, Sat 12:30pm-2pm'),
    },
  ];
  const events = [{ name: 'Camp', startDate: WED, endDate: THU }];
  const clashes = detectClashes(timetable, events);
  assert.deepEqual(clashes.map((c) => c.code), ['3', '4'], 'Chemistry Wednesday, then Maths Thursday');

  // Reported on the Tuesday, so Monday is already gone: both can only use Tue.
  const [chemistry, maths] = proposeMakeups(clashes, {
    lessons: timetable, events, today: TUE, nowMinutes: at(9),
  });

  assert.equal(maths.date, TUE, 'the centre gets its one slot');
  assert.equal(maths.start, at(17));
  assert.deepEqual(maths.displaces.map((d) => d.name), ['Japanese'], 'and Japanese gives way for it');
  assert.equal(chemistry.date, null, 'Chemistry goes without — it was never the constrained one');
});

test('a flexible lesson still gets a slot when it is not contested', () => {
  // The same two, but reported early enough that Monday is still available:
  // Maths takes its Tuesday hour, Chemistry takes the Monday.
  const timetable = [
    { code: '1', name: 'Japanese', weekday: 2, start: at(17), end: at(19), homeClass: true },
    { code: '3', name: 'Chemistry', weekday: 3, start: at(17, 45), end: at(19, 15) },
    {
  ...
  const events = [{ name: 'Camp', startDate: WED, endDate: THU }];
  const [chemistry, maths] = proposeMakeups(detectClashes(timetable, events), {
    lessons: timetable, events, today: SAT_BEFORE, nowMinutes: at(9),
  });

  assert.equal(maths.date, TUE);
  assert.equal(chemistry.date, MON, 'no need to contest the Tuesday');
  assert.equal(chemistry.displaces, undefined, 'and nothing is skipped for it');
});

test('proposals come back in the order they were given, however they were allocated', () => {
  // The message reads chronologically; only the allocation is re-ordered.
  const timetable = [
    { code: '3', name: 'Chemistry', weekday: 3, start: at(17, 45), end: at(19, 15) },
    { code: '4', name: 'Maths', weekday: 4, start: at(18, 30), end: at(20), makeupSlots: parseSlots('Tue 5pm') },
  ];
  const events = [{ name: 'Camp', startDate: WED, endDate: THU }];
  const clashes = detectClashes(timetable, events);
  const proposals = proposeMakeups(clashes, { lessons: timetable, events, today: SAT_BEFORE, nowMinutes: at(9) });

  assert.deepEqual(proposals.map((p) => p.code), clashes.map((c) => c.code));
});

test('when nothing fits, what stood in the way is named', () => {
  // The lesson blocking it is NOT a home class — which is the one thing the
  // person reading this can change, so it has to be said. Without it, a dead
  // end is indistinguishable from the home-class setting being ignored.
  const timetable = [LESSONS[0], CENTRE_MATH];
  const clashes = detectClashes(timetable, [{ name: 'Speech Day', startDate: THU }]);
  const [makeup] = proposeMakeups(clashes, {
    lessons: timetable, events: [], today: SAT_BEFORE, nowMinutes: at(9),
  ...
  assert.equal(makeup.date, null);
  assert.deepEqual(makeup.blockedBy.map((b) => b.name), ['Japanese']);
});

test('a lesson already ticked as a home class is not named as a blocker', () => {
  // It is not in the way — it is the thing being offered up.
  const timetable = [{ ...LESSONS[0], homeClass: true }, CENTRE_MATH];
  const clashes = detectClashes(timetable, [{ name: 'Speech Day', startDate: THU }]);
  const [makeup] = proposeMakeups(clashes, {
    lessons: timetable, events: [], today: SAT_BEFORE, nowMinutes: at(9),
  ...
  assert.ok(makeup.date, 'it is offered instead');
  assert.equal(makeup.blockedBy, undefined);
});
