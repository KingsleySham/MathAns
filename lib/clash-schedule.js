...
// ── Attending online ────────────────────────────────────────────────────────
//
// A centre that also runs the subject online usually runs it at other hours
// too, and those hours are no use to someone who has to be in the room. So a
// lesson may carry `onlineSlots` alongside its in-person ones, and they only
// join the search once online has been chosen FOR THAT LESSON — extra options,
// never replacements, and in person wins a tie. A proposal that came from the
// online list says so, because "Tue 5pm (online)" and "Tue 5pm" are not the
...
function conflictsOf({ date, from, to }, { lessons, windows, taken, skipCode }) {
  ...
  for (const t of taken) {
    if (t.date === date && overlaps(from, to, t.from, t.to)) blockers.push({ type: 'taken', name: t.name });
  }
  for (const lesson of lessons || []) {
...
// The times a lesson could be made up at, tagged with how it would be
// attended. Online hours only join the list once someone has opted in for that
// lesson — they are extra options, never a replacement for the centre's own.
function candidateSlots(lesson, { online }) {
  const inPerson = (lesson?.makeupSlots || []).map((s) => ({ ...s, online: false }));
  const virtual = online ? (lesson?.onlineSlots || []).map((s) => ({ ...s, online: true })) : [];
  return [...inPerson, ...virtual];
}

export function proposeMakeup(clash, {
  lessons, events, today, nowMinutes = 0, taken = [], online = false,
}) {
  ...
  // A centre's fixed hours (plus its online ones, if online was chosen) or —
  // when it has none on file — the lesson's own time, which is what a private
  // tutor moves to suit.
  const offered = candidateSlots(lesson, { online });
  ...
  let ifSkipped = null;
  // What stood in the way, when nothing worked. Without this a dead end is
  // silent, and a lesson that could have been ticked as a home class — which
  // would have made room — looks like the system ignoring the setting.
  const blockedBy = new Map();
  // Make-ups that got to a contested hour first. Not something to change —
  // just the difference between "nothing was free" and "another lesson needed
  // it more", which is worth knowing before ringing anyone.
  ...
  for (let date = addDays(clash.date, -1); date >= earliest; date = addDays(date, -1)) {
    ...
    for (const slot of slots) {
      ...
      for (const b of blockers) {
        if (b.type === 'lesson' && !b.lesson.homeClass) {
          blockedBy.set(b.lesson.code, { code: b.lesson.code, name: b.lesson.name });
        }
        if (b.type === 'taken' && b.name) takenBy.add(b.name);
      }
    }
  }
  ...
  return {
    ...
    // And by what — the lessons in the way that are not marked as home
    // classes, which is the one thing the person reading it can change.
    blockedBy: [...blockedBy.values()],
    takenBy: [...takenBy],
  };
}

// How many times a lesson could go at all this week, before anything else is
// considered. A centre that opens one hour before the clash has one; a tutor
// who moves to suit has one per remaining day.
function candidateCount(clash, { lessons, today, online }) {
  const lesson = (lessons || []).find((l) => l.code === clash.code);
  const offered = candidateSlots(lesson, { online });
  const monday = mondayOf(clash.date);
  const earliest = today && today > monday ? today : monday;
  let count = 0;
  for (let date = addDays(clash.date, -1); date >= earliest; date = addDays(date, -1)) {
    count += offered.length ? offered.filter((s) => s.weekday === weekdayOf(date)).length : 1;
  }
  return count;
}
...
//
// Which one gets first refusal on a contested hour matters, and "whichever
// clashed first" is the wrong answer. A lesson at a centre with fixed hours
// may have exactly one slot all week; a lesson whose tutor moves to suit has
// one every day. Handing the hour to whichever clash came first in the diary
// can leave the constrained one with nothing while the flexible one takes a
// slot it did not need — which is what happened with Maths (a centre, one
// Tuesday slot) losing out to Chemistry (flexible) for the same Tuesday.
//
...
// ALLOCATION changes — the proposals come back in the order they were given,
// so the message still reads chronologically.
export function proposeMakeups(clashes, { lessons, events, today, nowMinutes = 0, online } = {}) {
  const list = clashes || [];
  const isOnline = (code) => Boolean(online?.has?.(code) ?? (Array.isArray(online) && online.includes(code)));
  const rank = list.map((clash, i) => {
    const lesson = (lessons || []).find((l) => l.code === clash.code);
    return {
      i,
      fixedHours: lesson?.makeupSlots?.length ? 0 : 1,
      options: candidateCount(clash, { lessons, today, online: isOnline(clash.code) }),
      date: clash.date,
    };
  });
  rank.sort((a, b) => a.fixedHours - b.fixedHours
    || a.options - b.options
  ...
  const proposals = [];
  for (const { i } of rank) {
    const proposal = proposeMakeup(list[i], {
      lessons, events, today, nowMinutes, taken, online: isOnline(list[i].code),
    });
    if (proposal.date) {
      const lesson = (lessons || []).find((l) => l.code === list[i].code);
      taken.push({
        date: proposal.date, from: proposal.start, to: proposal.end, name: lesson?.name || list[i].code,
      });
    }
