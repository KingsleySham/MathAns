...
// "Mon 6pm", "Sat 2-4pm", "星期五 7:30pm" — one slot. Also accepts an object
// straight back out of Redis. Null when there is no weekday and time in it.
export function parseSlot(entry) {
  ...
  const m = /^\s*([A-Za-z一-鿿]+)\s*(.+)$/.exec(String(entry ?? ''));
  if (!m) return null;
  const weekday = parseWeekday(m[1]);
  if (weekday === null) return null;
  const [from, to] = m[2].split(/\s*(?:-|–|—|to)\s*/i);
...
// The whole list, typed as "Mon 6pm, Fri 6pm, Sat 2-4pm" or held as objects.
export function parseSlots(value) {
  const entries = Array.isArray(value) ? value : String(value ?? '').split(/[,;、\n]+/);
  return entries.map(parseSlot).filter(Boolean).slice(0, MAX_MAKEUP_SLOTS);
}
...
// True when a slot is clear of everything already on the calendar.
function isFree({ date, from, to }, { lessons, windows, taken, skipCode }) {
  if (windows.some((w) => w.date === date && overlaps(from, to, w.from, w.to))) return false;
  if (taken.some((t) => t.date === date && overlaps(from, to, t.from, t.to))) return false;
  return !(lessons || []).some((lesson) => {
    if (lesson.code === skipCode) return false;    // its own weekly slot is not a conflict
    const slot = lessonWindowOn(lesson, date);
    return slot && overlaps(from, to, slot.from, slot.to);
  });
}
...
export function proposeMakeup(clash, { lessons, events, today, nowMinutes = 0, taken = [] }) {
  ...
  for (let date = addDays(clash.date, -1); date >= earliest; date = addDays(date, -1)) {
    ...
    for (const slot of slots) {
      ...
      if (!isFree({ date, ...slot }, { lessons, windows, taken, skipCode: clash.code })) continue;
      return {
        code: clash.code,
