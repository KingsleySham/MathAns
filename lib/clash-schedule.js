...
function candidateCount(clash, { lessons, today }) {
  const lesson = (lessons || []).find((l) => l.code === clash.code);
  const offered = lesson?.makeupSlots || [];
  const monday = mondayOf(clash.date);
  const earliest = today && today > monday ? today : monday;
...
export function proposeMakeups(clashes, { lessons, events, today, nowMinutes = 0 } = {}) {
  ...
  const rank = list.map((clash, i) => {
    ...
    return {
      ...
      options: candidateCount(clash, { lessons, today }),
      date: clash.date,
    };
  });
  ...
  const proposals = [];
  for (const { i } of rank) {
    const proposal = proposeMakeup(list[i], { lessons, events, today, nowMinutes, taken });
    if (proposal.date) {
