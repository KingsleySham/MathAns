...
export function proposeMakeup(clash, { lessons, events, today, nowMinutes = 0, taken = [] }) {
  ...
  for (let date = addDays(clash.date, -1); date >= earliest; date = addDays(date, -1)) {
    const slot = { date, from: clash.start, to: clash.start + duration };
    // Too soon to be worth proposing — today, with the slot already passed or
    // about to.
    if (date === today && slot.from < nowMinutes + MIN_NOTICE_MINUTES) continue;
    if (!isFree(slot, { lessons, windows, taken, skipCode: clash.code })) continue;
    return {
      code: clash.code,
  ...
  return { code: clash.code, clashDate: clash.date, date: null, start: clash.start, end: clash.end };
}
