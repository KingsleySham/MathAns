...
// ── Attending online ────────────────────────────────────────────────────────
//
// A centre's fixed hours are a room and a teacher; online is neither, so it
// has no timetable to be constrained by. Choosing online for a lesson
// therefore does not add hours — it removes the constraint: the make-up may
// go at the lesson's own hour on ANY day in the window, exactly as a private
// tutor's would.
//
// The centre's in-person hours are still considered alongside, and in person
// wins a tie, because there is no reason to be online when the room is free.
// A proposal that ended up online says so, since "Tue 5pm (online)" and
// "Tue 5pm" are not the same instruction to anybody.
//
...
export function proposeMakeup(clash, {
  ...
  for (let date = addDays(clash.date, -1); date >= earliest; date = addDays(date, -1)) {
    ...
    // Online has no timetable, so it offers the lesson's own hour on any day —
    // which is the whole reason for choosing it. Added to whatever the centre
    // offers in person that day rather than replacing it.
    if (online || !offered.length) {
      slots.push({ from: clash.start, to: clash.start + duration, online: Boolean(online) });
    }
    // Nearest the usual hour first, and in person before online on a tie:
    // there is no reason to be online when the room is free.
    slots.sort((a, b) => Math.abs(a.from - clash.start) - Math.abs(b.from - clash.start)
...
function candidateCount(clash, { lessons, today, online }) {
  ...
  let count = 0;
  for (let date = addDays(clash.date, -1); date >= earliest; date = addDays(date, -1)) {
    count += offered.filter((s) => s.weekday === weekdayOf(date)).length;
    if (online || !offered.length) count += 1;   // its own hour, any day
  }
  return count;
}
