...
//
// The audience is a VHP decision, not an implementation detail: on a duty day
// it goes to every opted-in contact, using prefect_duty_reminder_weather. Easy
// to regress silently, since nothing fails loudly if the wrong people get
// messaged — hence this test.
//
// The fixture below deliberately has more contacts than roster names. In real
// use the contact list is the morning duty group (the VHP and two or three
// prefects, not the school's forty-strong board — see PREFECT_HUB.md), so the
// two sets nearly coincide; the extra contacts here exist to prove the send is
// driven by the contact list rather than by that day's roster names.
//
...
function seed() {
  store.clear();
  sends.length = 0;
  // A test that swaps in its own weather must not leak it into the next one.
  globalThis.fetch = baseFetch;
...
test('every recipient gets the day’s duty details and the warning', async () => {
  ...
  // Ruby is not among today's roster names and still gets the same duty
  // details — that is the point of sending to the contact list rather than the
  // roster row.
...
test('a duty marked Cancelled gets no weather update', async () => {
  seed();
  store.set('prefect:roster', JSON.stringify([
    { date: TODAY, location: 'Front gate', time: '7:45am', team: 'A', status: 'Cancelled', names: ['Calissa', 'Angelia'] },
  ]));

  const out = await morningCheck({ minutesNow: 7 * 60 });

  assert.equal(sends.length, 0, 'nobody should be told about a duty that is not happening');
