...
// One-off notice to the whole board or to one day's duty team, sent as text
// inside each recipient's 24h window and as the prefect_notice template
// outside it. This is the VHP speaking through the admin page — operational
// notices only, per the brief's Utility-category constraint.
export async function sendNotice({ text, audience = 'board', date } = {}) {
  ...
  for (const c of targets) {
    try {
      await sendTextOrTemplate(c.phone, body);
