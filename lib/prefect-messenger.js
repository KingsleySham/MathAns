...
import { handbookTally, looksLikeCode, verifyCode } from './prefect-handbook.js';
...
async function handleHandbookCode({ key, replyTo, name, text }) {
  const handbook = await readHandbook();
  if (!handbook.round) return false; // no round open — not a code, just a number

  const secret = await handbookSecret();
  if (verifyCode(secret, text, Date.now())) {
  ...
  // Wrong or expired. Say which, because silence here just becomes a message to
  // the VHP asking why nothing happened.
  await safeSendText(replyTo,
    'That code has expired or does not match. Open the handbook page again and send the '
    + '6-digit code showing there now — it changes every 10 minutes.');
  return true;
}
...
export async function handleVhpText({ from, fromUserId, profileName, text } = {}) {
  ...
  if (looksLikeCode(text)) {
    const sender = await resolveSender({ from, fromUserId, profileName });
    const handled = await handleHandbookCode({
