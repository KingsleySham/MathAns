...
import { SUSPEND, ADVISORY, CUTOFF, formatDayLabel, isCancelled } from './prefect-duty-status-logic.js';
import {
  extractCode, handbookTally, looksLikeCode, looksLikeCodeAttempt, normalizeCode, verifyCode,
} from './prefect-handbook.js';
import { handbookSecret, readHandbook, writeHandbook } from './prefect-handbook-store.js';
...
export async function handlePrefectText({ from, fromUserId, profileName, text }) {
  ...
  // A bare six-digit message is a handbook code, and nothing else in these
  // flows looks like one — an absence reason is prose. Checked first so a
  // prefect who is mid-absence-reason can still confirm the handbook, and it
  // only claims the message when a round is actually open.
  if (looksLikeCode(text) && await handleHandbookCode({ key, replyTo, name, text })) return;

  const awaiting = await getAwaiting(key);
  if (!awaiting) {
    // Not mid-absence-reason, so a message with any digits in it during an open
    // round is almost certainly a code attempt — mistyped, short, or pasted with
    // words around it. Catch it here rather than sending back the no-reply
    // redirect, which reads as a dead end to someone trying to do as asked.
    if (await handleHandbookAttempt({ key, replyTo, name, text })) return;
    return maybeRedirect(sender);
  }
...
// ══ handbook read-check ═════════════════════════════════════════════════════
//
// A rotating code sits on the handbook page (see prefects/handbook.html). The
// VHP starts a round, every opted-in prefect is asked to open the handbook, and
// sending the code back here records them as having read it.
//
// The code itself is in lib/prefect-handbook.js, along with an honest note on
// what it does and does not prove.
//
// The Redis state lives in lib/prefect-handbook-store.js, shared with the public
// status endpoint that serves the code to the embed.

export async function getHandbookStatus() {
  const [handbook, contacts] = await Promise.all([readHandbook(), getContacts()]);
  return { round: handbook.round, ...handbookTally(handbook, contacts) };
}

// Starts a round: clears the previous confirmations, rotates the code so any
// screenshot doing the rounds stops working, and asks everyone to go and read.
//
// Sent with the approved prefect_notice template, like every other
// business-initiated message here — a plain text would only reach prefects with
// an open 24-hour window.
export async function startHandbookRound({ url, note } = {}) {
  const link = String(url || '').trim();
  if (!link) return { ok: false, error: 'The handbook link is required — prefects need somewhere to go.' };

  await handbookSecret({ rotate: true });

  const text = [
    note && String(note).trim(),
    'Please read the Prefect Handbook before duty starts:',
    link,
    'A 6-digit code is shown on that page. Send it back to this number to confirm you have read it. '
    + 'The code changes every 10 minutes, so send it while the page is open.',
  ].filter(Boolean).join(' ');

  const out = await sendNotice({ text, audience: 'board' });
  ...
  return { ok: true, sent: out.sent, results: out.results };
}

// A six-digit message from a prefect, while a round is open. Returns false when
// it is not ours to handle, so the caller falls through to the normal flows.
const WRONG_CODE = 'Wrong code, try again. Open the handbook page and send the 6-digit code '
  + 'showing there now — it changes every 10 minutes.';

// Every reply here answers a message the prefect just sent, so their 24-hour
// window is open and plain text always delivers. Proactive messages — the round
// prompt, the duty reminders — go out as approved templates instead.
async function handleHandbookMessage({ key, replyTo, name, text, bareOnly }) {
  const handbook = await readHandbook();
  if (!handbook.round) return false; // no round open — not a code, just a number

  // Bare check for the pre-absence-reason pass; the looser one only runs once
  // we know the prefect is not mid-answer, so a reason containing a number is
  // never swallowed.
  const code = bareOnly
    ? (looksLikeCode(text) ? normalizeCode(text) : '')
    : extractCode(text);

  if (code && verifyCode(await handbookSecret(), code, Date.now())) {
    // Idempotent: sending it twice should read as reassurance, not an error.
    const already = Boolean(handbook.confirmed[key]);
    handbook.confirmed[key] = { name, at: new Date().toISOString() };
    await writeHandbook(handbook);
    await safeSendText(replyTo, already
      ? 'You are already down as having read the handbook — thanks for checking.'
      : 'Thanks — handbook confirmed. ✅');
    return true;
  }

  // A wrong code, or a fumbled one — a digit short, a stray letter, half a
  // paste. Both mean someone is trying to comply, and both used to fall through
  // to the "this is a no-reply number" brush-off, which tells them nothing.
  if (bareOnly ? Boolean(code) : looksLikeCodeAttempt(text)) {
    await safeSendText(replyTo, handbook.confirmed[key]
      ? 'You are already down as having read the handbook — no need to send it again.'
      : WRONG_CODE);
    return true;
  }

  return false; // nothing code-shaped about it; let the normal flows have it
}

const handleHandbookCode = (args) => handleHandbookMessage({ ...args, bareOnly: true });
const handleHandbookAttempt = (args) => handleHandbookMessage({ ...args, bareOnly: false });

// ── incoming: VHP commands ──────────────────────────────────────────────────

// The VHP is a prefect too — they do morning duty like everyone else — so
// anything a prefect can send here has to work from their number as well.
// Handbook codes are checked first for exactly that reason: the webhook routes
// the VHP down this path instead of handlePrefectText, and without this a code
// from their own phone gets the command help instead of being recorded.
//
// No ambiguity with the commands: a code is six digits and CANCEL is a word.
export async function handleVhpText({ from, fromUserId, profileName, text } = {}) {
  const t = String(text || '').trim().toUpperCase();

  if (t.startsWith('CANCEL')) return approveCancel();

  // Same treatment as a prefect: a code, or a fumbled attempt at one, is
  // answered rather than met with the command list. CANCEL is checked first
  // because it is the one word that must always reach the suspension flow.
  if (looksLikeCodeAttempt(text)) {
    const sender = await resolveSender({ from, fromUserId, profileName });
    const handled = await handleHandbookAttempt({
      key: sender.key || vhpPhone(),
      replyTo: sender.replyTo || vhpPhone(),
      name: sender.contact?.name || profileName || 'VHP',
      text,
    });
    if (handled) return;
  }
...
export async function sendReminders({ date } = {}) {
  ...
  // A duty marked Cancelled — in the editor or in Notion — still sits on the
  // roster and still shows on the calendar, but telling prefects to turn up for
  // it is worse than saying nothing. Suspensions are a different mechanism and
  // still go through the VHP.
  if (isCancelled(stored)) {
    return { date: target, sent: 0, results: [], note: 'duty cancelled' };
  }
...
//
// Goes to every opted-in contact, not just that day's roster names (VHP
// decision, Aug 2026). The contact list is the morning duty group — the VHP
// and two or three prefects, not the school's forty-strong board; see the
// scale note in PREFECT_HUB.md — so a warning that moves assembly indoors
// concerns all of them. Anyone rostered who declined gets it too.
//
// {{gate}}/{{time}}/{{day}} come from the day's roster entry and are the same
// for every recipient; only {{name}} differs. That holds while the contact
// list stays the duty group. If the hub is ever widened to prefects who don't
// do morning duty, the template's second-person wording needs revisiting in
// Meta — the repo can't enforce it from here.
//
// At most once per day, latched on prefect:wx-sent:{date}.
async function sendWeatherUpdate(stored, advisories) {
  if (await kvGet(`prefect:wx-sent:${stored.date}`)) {
    return { sent: 0, note: 'already sent today' };
  }
  const entry = await withDefaults(stored);
  const contacts = (await getContacts()).filter((c) => c.optIn && c.phone);
  if (!contacts.length) return { sent: 0, note: 'no opted-in contacts on file' };
...
//   • Advisory warning in force and duty today → send the weather update to
//     every opted-in contact (morning weather beats last night's guess).
...
export async function morningCheck({ minutesNow = hkMinutes() } = {}) {
  ...
  if (advisories.length && entry?.names?.length && !isCancelled(entry)) {
    const weatherUpdate = await sendWeatherUpdate(entry, advisories);
    return { held: false, weatherUpdate };
  }
  if (isCancelled(entry)) return { held: false, reason: "today's duty is cancelled" };
