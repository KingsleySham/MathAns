...
export function sanitizeLesson(raw) {
  ...
  const lesson = {
    ...
    // Whether the centre will take this lesson online. Online has no timetable
    // of its own — that is the point of it — so there is nothing to type but
    // yes or no. Chosen for a clash, it frees the make-up from the centre's
    // fixed hours; see lib/clash-schedule.js.
    //
    // Old records held the online hours as a slot list. Any lesson that had
    // one plainly runs online, so it carries across as a yes.
    canBeOnline: raw?.canBeOnline === true || parseSlots(raw?.onlineSlots).length > 0,
...
// parameter may not be empty (hence a dash in the unused slots) and it may not
// contain a newline (hence the ` · ` joins).
...
// The unused slots have to hold something — the Cloud API rejects an empty
// parameter — so they hold the quietest thing that is not empty.
export const NA = '—';

// Fills the seven fixed slots: one lesson each, a dash for the rest. An eighth
