...
import { eventWhen, isDate, makeupLabel, parseTime, parseWeekday, slotLabel } from './clash-schedule.js';
...
export const lessonLine = (lesson) => {
  ...
  return label ? `${head} → ${label}` : `${head} → no free slot, arrange with the tutor`;
};
