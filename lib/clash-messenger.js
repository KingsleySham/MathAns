...
import {
  ...
  SKIP_INSTRUCTION, doneTemplateParams, isClosed, lessonLine, markFixed, notifiable, parseCodes,
...
// The sign-off goes out as plain text unless a template is named. It used to
// default to `class_clash_done`, which meant a guaranteed-failing Cloud API
// call per recipient — and an error log for each — before falling back to the
// text anyway. Name a template here once one is approved in Meta.
const TPL_DONE = () => process.env.WHATSAPP_CLASH_DONE_TEMPLATE || '';
...
export async function sendDoneTemplate(caseRec, { day } = {}) {
  const recipients = notifiable(await getRecipients());
  const template = TPL_DONE();
  const { day: dayParam, ...bodyParams } = doneTemplateParams(caseRec, { day: day || dayLabel(hkNow()) });
  let sent = 0;
  for (const person of recipients) {
    const to = person.phone || person.userId;
    // Plain text, unless a sign-off template has been approved and named. The
    // text still falls back to the notice template when a window has closed,
    // so the news lands either way.
    if (template) {
      try {
        await sendTemplate({ to, name: template, headerParams: { day: dayParam }, bodyParams });
        sent += 1;
        continue;
      } catch (e) {
    ...
    try {
      ...
      console.error(`clash: done to ${person.name} failed:`, fallbackError.message);
    }
  }
...
export async function planClash({
  events = [], codes = null, makeups = null, skips = [], online = [], allowDisplacing = true,
} = {}) {
  ...
  const onlineFor = new Set((online || []).map((c) => String(c).toLowerCase()));
  const proposals = proposeMakeups(clashes, {
    lessons: catalogue, events: dated, today, nowMinutes: hkMinutes(), online: onlineFor,
  });
  const overrides = new Map((makeups || []).map((m) => [String(m.code).toLowerCase(), m]));

  const skipped = new Set((skips || []).map((c) => String(c).toLowerCase()));
  ...
  const lessons = clashes.map((clash, i) => {
    ...
    // A lesson that is not being made up at all carries no slot — only the
    // instruction to tell the tutor.
    if (skipped.has(clash.code)) proposal = null;
    return {
      ...lesson,
      date: clash.date,
      makeup: {
        ...
        online: proposal?.online === true,
      },
      skipped: skipped.has(clash.code),
      // Not part of the case — just what the page needs to explain a make-up
      // it could not place, and what to change if you want it placed.
...
export async function openClash({
  codes = null, makeups = null, skips = [], online = [], events = [], day,
  via = 'web', note = '', allowDisplacing = true,
} = {}) {
  const plan = await planClash({ events, codes, makeups, skips, online, allowDisplacing });
    ...
    try {
      await sendList(replyTo, {
        ...
        rows: pending.slice(0, MAX_PICK).map((l) => ({
          ...
          // "1️⃣ Japanese" — 24 characters is not much, so the slot, the
          // make-up and the skip note are all dropped from the title. What to
          // do about it goes in the description, which has room for it.
          title: lessonLine({ ...l, when: '', makeup: null, skipped: false }),
          description: l.skipped
            ? SKIP_INSTRUCTION
            : [l.when, l.tutor].filter(Boolean).join(' · '),
        })),
      });
    } catch (e) {
...
// Why a make-up could not be placed, in the terms of the one thing that can
// be changed about it — a lesson in the way that is not marked as a home
// class. "Arrange it with the tutor" is the answer only when nothing would.
function stuckReason(plan) {
  const inTheWay = [...new Map(
    (plan.lessons || [])
      .filter((l) => !l.makeup?.date)
      .flatMap((l) => l.blockedBy || [])
      .map((b) => [b.code, b]),
  ).values()];
  if (!inTheWay.length) return 'arrange those with the tutor.';
  const names = describeDisplaced(inTheWay);
  return `${names} ${inTheWay.length === 1 ? 'is' : 'are'} on then. `
    + `Tick ${inTheWay.length === 1 ? 'it' : 'them'} as a home class on the page if `
    + `${inTheWay.length === 1 ? 'it' : 'they'} could be skipped, or arrange with the tutor.`;
}
...
function planSummary(plan) {
  ...
  return [
    ...
    ...lines,
    stuck ? `\n${stuck} of them has no free slot that week — ${stuckReason(plan)}` : '',
    skipped
