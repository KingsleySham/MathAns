...
export async function planClash({ events = [], codes = null, makeups = null } = {}) {
  ...
  const lessons = clashes.map((clash, i) => {
    ...
    const proposal = overrides.get(clash.code) || proposals[i];
    return {
      ...lesson,
      date: clash.date,
      makeup: {
...
export async function openClash({ codes = null, makeups = null, events = [], day, via = 'web', note = '' } = {}) {
  const plan = await planClash({ events, codes, makeups });
...
async function handleClashText({ from, fromUserId, profileName, text }) {
  ...
  if (draft?.step === 'event') {
    ...
    await setDraft(key, { ...draft, step: 'confirm', events });
  ...
  if (draft?.step === 'confirm') {
    ...
    const out = await openClash({ events: draft.events || [], via: 'whatsapp' });
    if (!out.ok) {
