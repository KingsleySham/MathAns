...
test('every tutorial slot is filled, empty ones with N/A', () => {
  ...
  for (let i = 3; i <= TEMPLATE_SLOTS; i++) assert.equal(slots[`tutorial_${i}`], 'N/A');
});
...
test('the template lists what is outstanding and what is arranged', () => {
  ...
  assert.equal(params.tutorial_2, 'N/A');
...
test('the announcement names the lesson, who sorted it, and what is left', () => {
  const c = caseWith(['1', '2']);
  const lesson = markFixed(c, '1', 'Jessica', 'now');
  const text = fixedAnnouncement(c, lesson, 'Jessica');
  assert.match(text, /Japanese/);
  assert.match(text, /Jessica/);
  assert.match(text, /Piano/, 'the outstanding lesson must still be named');
});

test('the last announcement says nothing is outstanding', () => {
  const c = caseWith(['1']);
  const lesson = markFixed(c, '1', 'Kingsley', 'now');
  assert.match(fixedAnnouncement(c, lesson, 'Kingsley'), /Nothing else is outstanding/);
});
